...
lista<T>::lista():ancla(nullptr){}						///Definimos al ancla apuntando a nullptr
...
void lista<T>::insertarElem(nodo* p, const T& e){
    ...
    if(p==nullptr){				///Aqui se inserta al principio
        aux->setSig(ancla);
        ancla=aux;
    }
...
void lista<T>::eliminarElem(nodo* p){
    ...
    if(!PosValida(p)){
        throw Exception("Posicion Invalida, eliminarElem");
    }
